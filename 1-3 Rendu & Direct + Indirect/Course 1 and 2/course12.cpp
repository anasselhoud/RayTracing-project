...
{
    public:
        explicit Sphere(const Vector& O, double R, const Vector& albedo, bool isMirror = false, bool transp = false, bool creux = false, double n=1.2): O(O), R(R), albedo(albedo), isMirror(isMirror), transp(transp), creux(creux), n(n){
        };
...
int main() {
    ...
    Sphere S1plus(Vector(-10, -5, 15), 2, Vector(1., 0., 0.), true, false);
    Sphere S2(Vector(-10, -5, 30), 2, Vector(1., 0., 0.));
