...
public:
  ~TriangleMesh() {}
    TriangleMesh(const Vector& albedo, bool mirror = false, bool transp=false, double n=1.2, bool creux=false) {
    this->albedo = albedo;
        ...
        while(!l.empty()){
            ...
            if (courant->fg){
                ...
                for (int i = courant->start; i < courant->end; i++){
                    ...
                    double alpha = 1 - gamma - beta; // coordonnées barycentriques
